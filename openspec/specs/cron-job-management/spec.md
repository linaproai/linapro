# cron-job-management Specification
...
### Requirement: 代码与动态插件声明任务投影与只读治理

系统 SHALL 将宿主、源码插件以及动态插件声明的内置定时任务投影到统一的定时任务管理中,避免出现“部分任务只能在代码里看见、无法在管理页感知”的割裂体验。
...
#### Scenario: 动态插件内置任务统一可见

- **WHEN** 动态插件在运行时产物中声明独立的定时任务合同
- **THEN** 宿主 SHALL 在安装或启用链路中解析这些声明并同步到 `sys_job`
- **AND** 统一使用 `plugin:<pluginID>/cron:<name>` 形式的处理器引用接入共享 handler registry
- **AND** 管理员 SHALL 能在任务管理列表、详情与日志页面中查看和维护这些任务

#### Scenario: 动态插件未启用时保持可见但不可执行

- **WHEN** 动态插件的内置任务已经投影到 `sys_job`,但对应插件尚未启用或已被禁用/卸载
- **THEN** 系统 SHALL 将该任务状态标记为 `paused_by_plugin`
- **AND** 列表继续保留该任务与历史日志
- **AND** 前端 SHALL 禁止立即执行与启用类操作并明确提示插件当前不可用
...
- **WHEN** 系统初始化左侧导航菜单，或管理员后续在菜单管理中新增/编辑目录与菜单图标
- **THEN** 左侧导航中所有目录与菜单的图标 SHALL 在全局范围内唯一且不重复
- **AND** `定时任务` 目录、`任务管理`、`分组管理`、`执行日志` 的图标 SHALL 各不相同
- **AND** 系统管理、系统监控、系统信息等现有内置菜单也 SHALL 使用互不重复的图标
- **AND** 菜单管理保存与现有目录/菜单重复的图标时 SHALL 拒绝提交并提示明确错误,避免后续再次引入重复图标
