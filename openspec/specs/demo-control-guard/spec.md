...
系统 SHALL 将 `demo-control` 的已安装并启用状态视为演示保护的运行时开关。`plugin.autoEnable` 仅控制启动安装和启用；启动后不得将其视为单独的运行时开关。
...
系统 SHALL 交付名为 `demo-control` 的官方源码插件，使部署可通过启动配置或插件治理启用该能力。

#### Scenario:宿主发现 demo-control 源码插件
- **当** 宿主扫描源码插件并同步注册表数据时
- **则** 发现 `demo-control`
- **且** 运维人员可决定是否启用
...
启用时，demo-control SHALL 按 HTTP 方法语义阻断系统写请求，同时允许读式请求。
...
系统 SHALL 在 demo-control 启用时保留登录和退出行为，使演示环境保持可用。
...
#### Scenario:启用 demo-control 时拒绝插件安装
- **当** `demo-control` 已启用
