...
定义`LinaPro`仓库内统一管理各`AI Coding`工具`Agent`项目路径与`.agents/skills/`之间软链的开发工具能力边界。规范包含`make skills`/`make skills.link`/`make skills.unlink`公开入口、内置`vercel-labs/skills`官方项目路径映射表、`native`/`link`/`rootCollision`三类`Agent`分类、软链冲突保护规则、`TTY`下的交互式选择模式以及`Go`标准库实现的跨平台一致性约束。
