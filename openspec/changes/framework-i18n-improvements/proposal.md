...
`framework-i18n-foundation` 已经把宿主、插件和前端运行时 i18n 基础能力跑通,在 `zh-CN` 与 `en-US` 双语环境下功能完整。但落地过程中通过 78 条反馈逐步暴露出三类系统性问题:

1. **性能层**:`Translate` 热路径在每次调用时都会克隆 800+ key 的整张运行时消息包 (一次菜单列表请求 = ~100 次大 map 分配);缓存粒度是"全语言全扇区一锅清",任何插件启停都会击穿所有语言;运行时翻译包接口没有 ETag/版本戳,前端切语言每次都全量传输。
...
同时,当前框架只在 `zh-CN`/`en-US` 双语下被验证过;"约定式翻译键 + 缺失检查"在引入第三种语言时是否真的零业务代码改动,从未被压力测试。本次变更引入繁体中文 (`zh-TW`) 作为试金石,覆盖非拉丁字符和更完整的资源治理路径;文档方向按当前宿主定位固定为 LTR,不在本迭代支持 RTL。
...
- 把 `runtimeBundleCache` 重构为按 `locale + 扇区(host/source-plugin/dynamic-plugin)` 分层,失效时只清相关扇区或语言,不再整张清空。
...
### 固定 LTR 方向(纳入本次)
- `<html dir>` 与 `Ant Design Vue` 的 `ConfigProvider.direction` 按当前宿主约定固定为 `ltr`;默认配置不提供 `direction`。
- 验收标准:繁体中文下页面"内容正确、能用",不提供 RTL 镜像布局。

### 资源唯一事实源(纳入本次)
- 移除 `sys_i18n_locale` / `sys_i18n_message` / `sys_i18n_content` 三张运行时 i18n 持久化表,不再提供数据库翻译覆写、语言注册 seed 或通用业务内容多语言表。
- 保留导出、缺失检查和来源诊断 API 作为开发期/交付期辅助能力;导出结果用于离线校对并回写 JSON 资源,不是再导入数据库。
- 新增语言的唯一必需改动是宿主/插件/前端对应 JSON 资源,可选改动是默认配置文件中的 `i18n` 元数据。

### 范围外(后续独立变更)
- RTL 设计语言:图标镜像、抽屉/通知滑入方向、表格固定列翻转、CSS logical properties 全面替换、菜单展开方向。
- 用户级语言偏好沉淀到 `sys_user`。
- 国际化可视化管理后台页面。
- 线上热修翻译覆写能力;如未来需要,应作为可选插件独立设计。
- 自动机器翻译/AI 翻译辅助。
...
- `framework-i18n-foundation`: 翻译服务接口拆分为 `LocaleResolver` / `Translator` / `BundleProvider` / `Maintainer` 多个小接口;运行时翻译包接口新增 ETag/304 协商语义;新增 `zh-TW` 作为内置启用语言;运行时方向固定为 LTR;翻译资源加载在宿主、源码插件、动态插件之间共用统一资源加载器;运行时 i18n 持久化表被移除,翻译内容收敛为 JSON/YAML 资源唯一事实源。
- `system-api-docs`: API 文档翻译资源加载改用统一资源加载器,同步覆盖 `zh-TW`。
