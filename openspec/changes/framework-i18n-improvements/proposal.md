...
2. **一致性层**:5 个 `*_i18n.go` 适配文件每个都重新决定"何时翻译/何时跳过/用哪个 Translate*",导致 `dict` 在默认语言时 `return`、`menu` 一律 `Translate`、`sysconfig` 直接在 Go 里硬编码英文/中文 map(违反约定式翻译键决策);`isSourceTextBackedRuntimeKey` 把 jobmgmt 的 `job.handler.*` 命名前缀漏到 i18n 包里形成反向依赖;`Service` 接口承担 5 类职责共 18 个方法,业务模块只用前两类却必须 stub 整个接口。
...
### P2 一致性收敛
- 抽出 `i18n.LocaleProjector` 把 `menu_i18n.go`/`dict_i18n.go`/`sysconfig_i18n.go`/`jobmgmt_i18n.go`/`role.go` 内的"何时翻译/何时跳过/键如何拼接"统一管控,5 个文件 1134 行收敛到 ~300 行薄壳。
...
### P3 边界整理
- 抽出 `pkg/i18n/resourceloader` 通用资源加载器,接受 `Subdir` 与 `PluginScopeNamespace` 配置;`apidoc_i18n_loader.go` 与 `i18n_source.go` 共用同一份实现,删除 ~280 行重复代码。
...
- `framework-i18n-projector`: 业务模块统一的本地化投影组件,封装翻译键推导、跳过策略与 fallback 选择,让 `*_i18n.go` 适配文件薄到只剩调用入口。
...
- **后端能力**:重写 `apps/lina-core/internal/service/i18n/` 内的 `Translate*` 热路径与缓存层;拆分 `Service` 接口;抽出 `pkg/i18n/projector`、`pkg/i18n/resourceloader`、`pkg/pluginbridge/wasm.go`;调整 5 个 `*_i18n.go` 适配文件;`apidoc_i18n_loader.go` 与 `i18n_source.go` 共用 loader;`config-management` 删除 Go 硬编码标签 map。
