...
- [x] 2026-05-12: FB-20 验证通过:`gofmt -w apps/lina-core/pkg/dialect/internal/sqlite/translate.go apps/lina-core/pkg/dialect/internal/sqlite/translate_test.go`;`cd apps/lina-core && go test -count=1 ./pkg/dialect/internal/sqlite`;`cd apps/lina-core && go test -count=1 ./pkg/dialect`;`rg -n "skip PostgreSQL comment statement during SQLite translation" apps/lina-core/pkg/dialect/internal/sqlite apps/lina-core/pkg/dialect`;`openspec validate cross-platform-dev-commands --strict`;`git diff --check -- apps/lina-core/pkg/dialect/internal/sqlite/translate.go apps/lina-core/pkg/dialect/internal/sqlite/translate_test.go openspec/changes/cross-platform-dev-commands/tasks.md`。确认 SQLite SQL 翻译器跳过 `COMMENT ON TABLE/COLUMN` 元数据时不再输出 debug 日志,仍继续静默丢弃 PostgreSQL 注释元数据并保留兼容的索引维护语句。i18n 影响:本次仅修改后端 SQLite SQL 翻译器内部日志行为、单元测试和 OpenSpec 任务记录,不新增运行时菜单、按钮、表单、接口消息、插件 manifest 文案、运行时 i18n JSON、manifest i18n 或 apidoc i18n。缓存影响:不新增或修改业务运行时缓存、缓存键、缓存失效路径或分布式缓存一致性逻辑。
- [x] 2026-05-12: FB-20 `/lina-review` 审查通过:审查范围限定为 `apps/lina-core/pkg/dialect/internal/sqlite/translate.go`、`apps/lina-core/pkg/dialect/internal/sqlite/translate_test.go` 与本任务记录。实现只移除 `COMMENT ON TABLE/COLUMN` 跳过分支的 debug 输出,未改变 SQLite SQL 翻译结果或 unsupported SQL 拒绝策略;保留的 PostgreSQL procedural block debug 日志仍使用传入的 `ctx`。测试删除全局 logger handler 捕获,避免共享日志状态影响并保持自包含、可并行运行;目标测试继续覆盖注释元数据被丢弃和索引维护语句被保留。该变更不涉及 REST API、SQL 交付文件、前端 UI、运行时 i18n、数据权限、业务缓存或分布式缓存一致性。
- [x] **FB-21**: GitHub Actions 使用 Go 1.25 时根 `go.work` 声明 Go 1.26.2 导致 CI Go 命令提前失败
- [x] 2026-05-12: FB-21 验证通过:`GOTOOLCHAIN=local go run ./hack/tools/linactl help`;`GOTOOLCHAIN=local go run ./hack/tools/linactl status`;`GOTOOLCHAIN=local go run ./hack/tools/linactl prepare-packed-assets`;`GOTOOLCHAIN=local go run ./hack/tools/linactl wasm dry-run=true`;`GOTOOLCHAIN=local make init confirm=init`;`GOTOOLCHAIN=local ./hack/tests/scripts/run-sqlite-smoke.sh`;`GOTOOLCHAIN=local go test ./hack/tools/linactl`;`GOTOOLCHAIN=local go list -m -f '{{.Dir}}'`;`GOTOOLCHAIN=local go run github.com/rhysd/actionlint/cmd/actionlint@latest .github/workflows/*.yml`;`openspec validate cross-platform-dev-commands --strict`;`git diff --check -- go.work openspec/changes/cross-platform-dev-commands/tasks.md .github/workflows`。确认根 `go.work` 的 Go 版本声明已与仓库所有 `go.mod` 和 GitHub Actions `go-version: "1.25"` 基线一致,避免 CI 在仓库根执行 `go run ./hack/tools/linactl ...`、`make init` 或 SQLite smoke 时因 workspace 版本高于已安装工具链而提前失败。补充验证:`GOTOOLCHAIN=local go run ./hack/tools/linactl test-go race=false verbose=false` 暴露当前工作区另有未提交 Redis/cluster 配置改动导致的测试失败,包括 `config.Service` fake 缺少 `GetClusterRedis`、cluster 测试缺少 `cluster.coordination` 配置以及 panic allowlist 未更新;这些文件不属于本次 GitHub Actions 失败提交 `e36fc31` 的修复范围。i18n 影响:本次仅修改 Go workspace 工具链声明和 OpenSpec 任务记录,不新增运行时菜单、按钮、表单、接口消息、插件 manifest 文案、运行时 i18n JSON、manifest i18n 或 apidoc i18n。缓存影响:不新增或修改业务运行时缓存、缓存键、缓存失效路径或分布式缓存一致性逻辑。
- [x] 2026-05-12: FB-21 `/lina-review` 审查通过:审查范围限定为 `go.work` 与本任务记录。`go.work` 仅将 workspace Go 版本声明从 `1.26.2` 调整为 `1.25.0`,与所有模块 `go.mod` 和 `.github/workflows/*.yml` 中的 `go-version: "1.25"` 保持一致;不修改 workflow 行为、业务源码、REST API、SQL、前端 UI、运行时 i18n、数据权限、业务缓存或分布式缓存一致性。当前工作区存在的 Redis/cluster 相关未提交文件和测试失败不属于本次 GitHub Actions 失败修复范围。
- [x] **FB-22**: Nightly test workflow should be integrated into nightly build workflow so image publishing only runs after tests pass
- [x] 2026-05-12: FB-22 验证通过:`go run github.com/rhysd/actionlint/cmd/actionlint@latest .github/workflows/*.yml`;`ruby -e 'require "yaml"; Dir[".github/workflows/*.yml"].each { |f| YAML.load_file(f); puts f }'`;`openspec validate cross-platform-dev-commands --strict`;`git diff --check -- .github/workflows openspec/changes/cross-platform-dev-commands/tasks.md`。确认 `.github/workflows/nightly-test.yml` 已整合进 `.github/workflows/nightly-build.yml`,保留 Windows command smoke、Go unit tests、Frontend unit tests 与 Full E2E tests 四个测试 job;`nightly-image` 通过 `needs` 显式依赖全部测试 job,仅当测试全部成功时才构建并发布 nightly 镜像。全局 workflow 权限保持 `contents: read`,仅发布镜像 job 追加 `packages: write`。i18n 影响:本次仅修改 GitHub Actions workflow 和 OpenSpec 任务记录,不新增运行时菜单、按钮、表单、接口消息、插件 manifest 文案、运行时 i18n JSON、manifest i18n 或 apidoc i18n。缓存影响:不新增或修改业务运行时缓存、缓存键、缓存失效路径或分布式缓存一致性逻辑。
- [x] 2026-05-12: FB-22 `/lina-review` 审查通过:审查范围限定为 `.github/workflows/nightly-build.yml`、已删除的 `.github/workflows/nightly-test.yml` 与本任务记录。合并后的 nightly build workflow 使用单一定时入口,测试 jobs 与镜像发布 job 位于同一 workflow run 中;GitHub Actions 默认语义会在任一 `needs` job 失败或取消时跳过 `nightly-image`,避免发布未通过测试的产物。删除独立 nightly test workflow 后不会再出现测试与构建两个 nightly run 相互独立调度的问题。该变更不涉及 REST API、SQL、Go 业务实现、前端 UI、运行时 i18n、数据权限、业务缓存或分布式缓存一致性。
