...
- Replace generated and internal `*gtime.Time` types with `*time.Time` via GoFrame DAO generation configuration.

**Non-Goals:**

- Do not add a static scanning tool or automatic check command.
- Do not change database fields or GoFrame-generated `dao/entity` structure definitions.
- Do not add runtime configuration, cache, permission, or data-permission logic.

## Decisions

### Instant fields use Unix timestamps in milliseconds
...
### The rule applies only at the response DTO boundary
...
### Calendar dates are an explicit exception
...
### Manual review gate, no static scanner

This change does not introduce a static scanning tool. `lina-review` checks API DTO changes for time-field type, unit documentation, calendar-date exception documentation, and frontend, `apidoc i18n`, and test impact when existing field types are migrated.

### Existing API migration uses a shared projection helper
...
### GoFrame DAO generation uses standard library time

Core and source-plugin GoFrame DAO generation configs use `stdTime: true` plus timestamp `typeMapping` entries so regenerated PostgreSQL timestamp fields use `*time.Time` instead of `*gtime.Time` and preserve database NULL semantics. This prevents future regenerated timestamp fields from reverting to `gtime`.
