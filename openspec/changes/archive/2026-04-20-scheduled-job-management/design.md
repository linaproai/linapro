...
- 动态插件安装/启用前的宿主服务授权审查页需要额外投影 `cron` host service 已发现的定时任务摘要,至少展示任务名称、表达式、调度范围与并发策略,避免管理员在确认授权时看不到将要注册的任务内容。
- 授权审查页中的宿主服务排序需要把 `runtime` 固定压到最后一张卡片; `cron` 卡片不再展示单独的“定时任务”摘要标签,而是直接以任务面板列出明细,并对属性标题做粗体强调,降低信息层级噪音。
- 授权页与详情页复用同一套宿主服务资源标签视觉语义: “申请清单”在授权页使用与详情页“生效范围”一致的标签底色,资源类型标签与 cron 任务面板背景也保持一致,避免同一资源在两个视图里出现不同语义色。
