...
E2E 测试套件 SHALL 保留 host-only 与 plugin-full 的语义区分。Release 完整插件发布 SHALL 同时运行 host-only E2E 和 plugin-full E2E；plugin-full E2E 不得被 host-only E2E 替代，官方插件工作区缺失时不得静默降级为 host-only E2E。

#### Scenario: Release 同时运行 Host-only 和 Plugin-full E2E
- **WHEN** release workflow 执行发布前 E2E 门禁
- **THEN** workflow SHALL 运行 host-only E2E 入口
- **AND** workflow SHALL 运行 plugin-full E2E 入口
- **AND** release 镜像发布 job SHALL 等待两个 E2E job 均成功
