...
系统 SHALL 要求宿主 `init` 和 `mock` 命令在执行任何 `SQL` 之前接收与命令名一一对应的显式确认值；未提供或提供错误确认值时，命令 MUST 拒绝执行。`init` 与 `mock` 只允许作用于宿主 bootstrap 初始化资产，不承担正式升级语义，也不得执行业务系统运行时升级资产或其他正式升级计划。
...
- **THEN** 命令允许进入对应的 bootstrap `SQL` 扫描与执行流程
- **AND** 命令不得把正式升级资产目录纳入扫描范围
