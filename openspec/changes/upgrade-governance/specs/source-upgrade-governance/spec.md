...
### Requirement: 框架必须提供正式的源码升级命令
框架 SHALL 提供独立于 `init` 和 `mock` 的正式源码升级命令，用于二开项目从旧框架版本升级到新框架版本。
...
### Requirement: 源码升级必须基于当前项目框架版本比较目标版本
框架 SHALL 以当前项目 `apps/lina-core/manifest/config/metadata.yaml` 中记录的框架版本号作为当前项目的升级基线，并与目标标签代码中的目标框架版本号进行比较；若目标版本不高于当前版本，则命令 MUST 安全退出且不得执行覆盖或 SQL。

#### Scenario: 目标版本不高于当前版本
- **WHEN** 升级命令解析出的目标框架版本小于或等于当前项目 `metadata.yaml` 中记录的框架版本
