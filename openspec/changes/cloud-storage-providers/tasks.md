...
- [x] **FB-11**: 去掉宿主「存储管理 / 云存储」一级目录；所有云存储配置菜单挂载到「系统设置」（`parent_key: setting`）
- [x] **FB-12**: 「S3 存储」菜单名称改为「S3存储管理」
- [x] **FB-13**: 「S3存储管理」菜单名称改为「存储管理-S3」
- [x] **FB-14**: 全部云存储插件「测试连接」失败改为 Modal.error 弹窗展示详情（对齐邮件管理），移除页面顶部错误 Alert；更新 S3 TC003
