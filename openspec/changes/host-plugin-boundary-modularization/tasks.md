...
- [x] **FB-34**: 日志 TraceID 默认应关闭，并同时支持 `config.yaml` 静态开关与系统参数动态覆盖
  - 2026-04-22：宿主新增 `logger.extensions.traceIDEnabled` 静态配置项与受保护系统参数 `sys.logger.traceID.enabled`（支持 `inherit|true|false`），日志默认不再输出 TraceID；运行时通过 `config.Service.IsLoggerTraceIDEnabled` 驱动自定义 logger handler 动态裁剪 TraceID，并已回归 `go test ./pkg/logger -run 'Test(ConfigureInstallsCustomHandler|StructuredTraceIDAwareHandlerDropsTraceID|TextTraceIDAwareHandlerKeepsTraceID|BindServerAlignsSharedOutput|BindServerUsesDefaultPatternWhenFileMissing)'`、`go test ./internal/service/config -run 'Test(GetLoggerUsesStructuredSwitch|GetLoggerUsesDefaultWhenSwitchMissing|ValidateRuntimeParamValue|IsLoggerTraceIDEnabledUsesStaticConfigFallback|IsLoggerTraceIDEnabledUsesRuntimeOverride|IsLoggerTraceIDEnabledSupportsRuntimeEnableOverride|IsLoggerTraceIDEnabledSupportsInheritMode)'`、`go test -run '^$' ./internal/service/role` 与 `pnpm exec playwright test e2e/system/TC0079-config-runtime-params.ts --config=/tmp/pw-chrome.config.ts --project=chrome`
- [x] **FB-35**: 系统菜单类型在宿主代码中仍散落使用 `"D"` / `"M"` / `"B"` 字符串，需收敛为强类型常量并统一复用
...
- [x] **FB-36**: `monitor-operlog` 仍使用 `1~6` 整数表达操作类型，需改为 `create/update/delete/export/import/other` 语义字符串并同步宿主事件、插件接口、字典与前端契约
...
- [x] **FB-37**: 宿主工作台仍残留未引用的旧版 `monitor-operlog` 前端页面与 API 副本，需删除重复实现并回归操作日志相关 E2E，避免后续误接回旧契约
  - 2026-04-22：已删除 `apps/lina-vben/apps/web-antd/src/{api/monitor/operlog,views/monitor/operlog}/` 旧宿主页副本，当前仅保留 `apps/lina-plugins/monitor-operlog/frontend/pages/` 作为前端事实来源；`rg` 已确认宿主源码中无残留引用。最初因本地缺少可用 Playwright 浏览器产物未能完成正式回归；现已补跑并通过受影响用例 `TC0026`、`TC0027`、`TC0028`、`TC0029`、`TC0035` 以及插件生命周期回归 `TC0098`（命令：`cd hack/tests && npx playwright test e2e/monitor/TC0026-operlog-list.ts e2e/monitor/TC0027-operlog-detail.ts e2e/monitor/TC0028-operlog-clean.ts e2e/monitor/TC0029-operlog-export.ts e2e/monitor/TC0035-operlog-delete.ts e2e/plugin/TC0098-official-source-plugin-lifecycle.ts --project=chromium`，20/20 通过）

- [x] **FB-38**: `pluginbridge` 暴露统一的 guest controller 脚手架——新增泛型 `BindJSON[T](request)`、`WriteJSON(statusCode, payload)`、envelope 路径与查询访问器（`PathParam` / `QueryValue` / `QueryInt` / `QueryFlag`），以及 `ErrorClassifier` + `NewErrorClassifier(ErrorCase...)` 组合器，并补齐单元测试覆盖正常 / 空 body / 非法 JSON / marshal 失败 4 条路径
...
- [x] **FB-39**: `plugin-demo-dynamic` 控制器样板瘦身——删除 `dynamic_request.go` 与所有 `buildXxxRouteInput` / `readDemoRecordIDFromXxxRoute` 重复包装，全部路由改用 `pluginbridge` 新 helpers，`buildDynamicErrorResponse` 改为 `ErrorClassifier` 组装；验证 `go test ./pkg/pluginbridge/...`、`GOOS=wasip1 GOARCH=wasm go build`、`TC0067-runtime-wasm-lifecycle` 与重构前后 `.wasm` 产物尺寸
...
- [x] **FB-40**: 动态插件 guest controller 支持 API DTO 驱动的 typed req/res 签名——让 `plugin-demo-dynamic` 路由控制器可直接使用 `context.Context + *v1.Req -> *v1.Res` 形式，并补齐 `backend-summary` / `host-call-demo` API DTO 与实际输入输出对齐
...
- [x] **FB-41**: 既然上游 `github.com/gogf/gf/v2` 已修复 Wasm 下 `gdebug.init.0` 对空 `os.Args` 的 panic，则移除动态插件 API DTO 的 `apimeta.Meta` 兼容层，并重新验证 typed guest controller 与动态插件 Wasm 运行回归
...
- [x] **FB-42**: 移除 `sys.logger.traceID.enabled` 参数配置，日志 TraceID 开关仅允许由 `config.yaml` 中的 `logger.extensions.traceIDEnabled` 控制
  - 2026-04-22：已删除宿主 `config.Service.IsLoggerTraceIDEnabled` 运行时覆盖链路与 `sys.logger.traceID.enabled` 受保护参数定义，`main.go` 与 `pkg/logger` 改为仅消费 `logger.extensions.traceIDEnabled` 静态布尔开关；`007-config-management.sql` 改为初始化时清理旧参数且不再插入新记录，`TC0079` 同步改为断言该参数不可检索。为恢复本地验证链路，补齐了 `apps/lina-plugins/monitor-server/go.mod` / `go.sum` 的 `gopsutil` 依赖并更新 `go.work.sum`。验证通过：`cd apps/lina-core && go test ./pkg/logger ./internal/service/config ./internal/service/sysconfig ./internal/service/role`、`cd apps/lina-core && go build ./...`、`cd apps/lina-plugins/monitor-server && go test ./...`、`make init confirm=init` 后以 MySQL 与 `/api/v1/config` 查询确认 `sys.logger.traceID.enabled` 已不存在，同时通过 headless Playwright 手工脚本校验 `#/system/config` 页面仍可检索 `sys.jwt.expire` 且不可检索 `sys.logger.traceID.enabled`

- [x] **FB-43**: 统一任务调度接口文档分组名称，改为 `任务调度/定时任务`、`任务调度/任务分组`、`任务调度/执行日志`、`任务调度/插件处理器`
  - 2026-04-22：已更新 `apps/lina-core/api/job*/v1/*.go` 中相关 `g.Meta tags`，并通过全文检索确认旧分组标签已清理完毕；该反馈仅影响接口文档分组展示，不涉及运行时行为，未新增 E2E。
