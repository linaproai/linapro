...
- 扩展`linapro-ai-core`元数据模型，支持通用 provider endpoint、模型身份、能力方法档位、能力方法默认参数、最小调用日志和 provider operation 投影。
...
- `linapro-ai-core-multimodal-plugin`：定义`linapro-ai-core`智能中心对多模态 provider、endpoint、模型身份、档位、默认参数、调用日志、缓存、管理页面和 provider adapter 的扩展要求。
...
- 影响智能中心插件数据库：需要为 provider endpoint、模型身份、方法档位、方法参数、调用日志和 provider operation 投影设计幂等 SQL、索引和缓存失效。
