...
`add-ai-text-capability-center`和`refactor-ai-capability-namespace`已经建立了`AI().Text()`、`framework.ai.text.v1`和`linapro-ai-core`智能中心的基础。下一步需要把图片、向量、音频、视觉、文档、安全审核和视频等能力纳入同一个可治理的`AI`能力体系，避免业务模块直接感知供应商协议、模型差异、密钥和调用审计细节。

本次设计也需要明确边界：`lina-core`和`linapro-ai-core`只管理`AI`能力、元数据、供应商模型适配和最小调用治理，不承载“生成视频任务”“业务后台队列”“用户进度通知”等具体业务场景实现。
...
- 为供应商异步协议定义`ProviderOperationRef`边界。`linapro-ai-core`可以适配供应商的`create -> poll -> download`协议，但业务异步任务、队列、状态、重试、通知和资产归属 MUST 由调用方业务模块实现。
- 扩展`linapro-ai-core`元数据模型，支持通用 provider endpoint、模型能力声明、能力方法档位、能力方法默认参数、最小调用日志和 provider operation 投影。
...
- 智能中心管理页面从文本能力扩展为能力方法分组配置，但不新增具体业务任务页面，例如不提供`/ai/video-jobs`。
- 本轮实现范围建议分期：优先完成通用契约、元数据重构、host service DTO 和 OpenAI/Anthropic/Voyage 等 provider adapter 的窄实现；视频 adapter 可先完成协议形态和能力声明，不强制完整业务编排。
...
- `linapro-ai-core-multimodal-plugin`：定义`linapro-ai-core`智能中心对多模态 provider、endpoint、模型能力声明、档位、默认参数、调用日志、缓存、管理页面和 provider adapter 的扩展要求。
...
- 影响智能中心插件数据库：需要为 provider endpoint、模型能力、方法档位、方法参数、调用日志和 provider operation 投影设计幂等 SQL、索引和缓存失效。
- 影响 API 契约：新增或修改供应商、模型、档位、能力方法、调用日志、provider operation 查询等管理 API，必须满足 REST 语义、分页/上限、Unix 毫秒时间和完整文档标签。
...
- 影响前端和 E2E：智能中心页面需要支持能力方法分组、provider endpoint 管理、多模态模型能力声明和资产/operation 摘要展示，并补充对应 E2E 覆盖。
