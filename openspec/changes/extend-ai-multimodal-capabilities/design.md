...
- `linapro-ai-core`负责供应商、模型、档位、调用日志和 provider adapter，不把具体存储回流到宿主。

本次变更在这个基础上扩展多模态能力。需要覆盖的能力包括图片、向量、音频、视觉、文档、安全审核和视频；明确排除`computer.act`。视频需要特别处理：供应商可能采用异步生成协议，但框架和智能中心不能变成业务任务中心。业务异步任务、队列、进度展示、通知、重试和资产归属必须由调用`AI`能力的业务模块负责。

外部能力口径截至`2026-06-02`：OpenAI 具备图片、音频、视觉、文件输入、安全审核和 Sora 视频相关 API；Anthropic/Claude 具备视觉、PDF、工具类能力，但未看到原生视频生成 API；Voyage 等供应商支持多模态 embedding。供应商能力变化快，因此平台契约必须按 provider-generic 方法设计，而不是绑定某一家供应商的长期产品形态。
...
- 定义`ProviderOperationRef`，让`linapro-ai-core`能适配供应商异步协议，但不实现具体业务任务。
- 将`linapro-ai-core`供应商端点从固定列重构为可扩展 endpoint 模型，支撑 OpenAI-compatible、Anthropic-compatible、Voyage-compatible 等协议。
- 扩展模型能力声明、能力方法档位、默认参数、调用日志、缓存失效、平台权限和`i18n`治理。
...
- 不要求所有 provider 都实现所有能力；能力可用性由模型能力声明、档位绑定和 provider adapter 决定。
- 不在调用日志中保存完整输入、完整输出、音视频原文、文件内容、供应商响应原文或密钥。
...
`capabilityType + capabilityMethod`仍然是档位、模型能力、日志和授权的核心身份。`vision.analyze`和`document.analyze`不塞进`text.generate`，避免文本能力被多模态 content block 侵蚀。
...
`assetRef`可以是宿主文件能力、插件资产能力或受控临时对象的引用。具体实现应复用现有文件/存储能力接缝，不把二进制内容放入 host service JSON。对于临时 URL 或 provider download URL，必须先转换为受控引用或受控临时资产投影，避免把供应商认证 URL 暴露给插件或前端。

### 4. 供应商异步只暴露`ProviderOperationRef`

视频和部分音频/图片供应商可能不是同步完成。`linapro-ai-core`可以保存最小 provider operation 投影：
...
这使 AI 层保持“供应商协议适配和能力治理”职责，不承载“生成某类业务视频”这样的场景逻辑。
...
provider 主表只保存供应商元数据；协议端点、基础地址和密钥引用必须从初始设计开始收敛到独立 endpoint 表：
...
```text
plugin_linapro_ai_model_capability
  model_id
  capability_type
  capability_method
  input_modalities
...
`supports_operation`表示 provider 方法可能返回`ProviderOperationRef`。`supports_thinking`和`supported_efforts`只对`text.generate`等具备推理强度语义的方法生效，仍属于方法级能力声明。模型基础表只保存供应商、默认 endpoint、模型名称、协议、来源和启用状态等身份字段，不再保存`capability_type`、`capability_method`、token 上限或`thinkingEffort`支持范围，避免模型表和能力表形成双事实源。

模型能力声明不自动推断；同步模型只写入供应商公开返回且平台能确认的能力，其他能力由管理员手工维护。供应商模型列表、档位候选模型查询、档位绑定校验和运行时解析缓存均以`plugin_linapro_ai_model_capability`为能力方法唯一事实来源，并使用`model_id + capability_type + capability_method`唯一键完成集合化装配。
...
资源策略用于约束`purpose`、最大输出、最大输入资产数量、最大字节数、允许 mime 类型、默认档位、是否允许 provider operation 和是否允许取消。任一校验失败必须在读取密钥或调用供应商前拒绝。
...
- 缓存内容：启用档位、主绑定、provider endpoint、模型能力、默认参数和 secret 引用，不缓存密钥明文。
- 失效触发：provider、endpoint、model、model capability、tier、binding 和默认参数写入成功后。
...
- [Risk] `ProviderOperationRef`可能被误用为业务 job。→ Mitigation：规格明确 operation 只代表供应商协议状态，业务模块必须自建任务和业务归属。
- [Risk] 不同 provider 的多模态能力差异很大。→ Mitigation：模型能力声明以方法为中心，adapter 只实现已声明能力，不支持时返回结构化不可用错误。
...
- [Risk] 视频供应商能力变化快。→ Mitigation：视频能力按 provider-generic 方法和 operation 引用设计，不绑定 Sora 长期契约。

## Implementation Plan

1. 在现有`AI().Text()`基础上新增子能力包和公共值对象，保持文本能力 ID 和行为不变。
2. 扩展`pluginbridge`host service 常量、校验和 guest SDK，新增多模态`ai.*`方法，但默认未授权。
3. 修改`linapro-ai-core`插件 SQL：新增 provider endpoint、model capability、method 参数和 operation 投影表；provider 主表不包含固定端点列或密钥引用列。
