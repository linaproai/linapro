...
租户管理页面 SHALL 在每个租户行内提供代操作和用户管理入口;代操作按钮 SHALL 在鼠标悬浮时展示简短说明,用户管理入口 SHALL 跳转到系统用户管理页面并自动按当前租户筛选用户。
...
#### Scenario: 跳转租户用户管理
- **WHEN** 平台管理员在租户列表点击租户 A 的用户管理按钮
- **THEN** 前端跳转到 `/system/user?tenantId=A`
- **AND** 用户管理页顶部租户筛选下拉框自动选中租户 A 的名称
- **AND** 用户列表通过 `GET /user?tenantId=A` 按该租户 active membership 过滤
