...
## 4. 新建 linapro-extlogin-core 插件骨架

- [x] 4.1 创建 `apps/lina-plugins/linapro-extlogin-core/` 目录结构（`plugin.yaml`、`plugin_embed.go`、`go.mod`/`go.sum`、`Makefile`、`hack/config.yaml`、`backend/plugin.go`、`manifest/`），参照 `linapro-demo-source`；无 `frontend/`（capability-only，embed 仅 `plugin.yaml manifest`）
- [x] 4.2 `plugin.yaml` 声明 `type: source`、`distribution: builtin`、`platform_only`/`global`、`i18n` 配置（en-US 默认 + zh-CN）；无独立菜单（仅提供能力 + 自隔离 API）
- [x] 4.3 建表 SQL 与卸载 SQL 落地：`plugin_linapro_extlogin_core_user_external_identity`（字段按 D2；软删 `deleted_at` + **部分唯一索引** `WHERE deleted_at IS NULL`，解绑释放 `(provider,subject)` 键以支持重绑；`idx_plugin_linapro_extlogin_core_identity_user` 支撑 List/Unbind；全部 `IF NOT EXISTS` 幂等；无自增 id 显式写入；无 Seed DML）。卸载 SQL 仅 `DROP TABLE`，**不级联删除** `sys_user` 孤儿账号（7.5 语义，注释记录）
- [x] 4.4 插件 `hack/config.yaml` DAO 生成配置 + `make dao dir=apps/lina-plugins/linapro-extlogin-core/backend` 生成 DAO/DO/Entity（先经临时 SQL 执行器建表，执行后即删）
...
- [x] 5.6 `manifest/i18n/{zh-CN,en-US}/error.json` 落地 9 个错误码本地化（键由 `PLUGIN_EXTLOGIN_CORE_*` 派生）
...
- [x] 6.1 `linapro-oidc-google`/`linapro-oidc-discord` 的 `plugin.yaml` 增加 `dependencies.plugins: [{id: linapro-extlogin-core, version: ">=0.1.0"}]`。**D5 实施校正**：两插件维持 `distribution: managed`（带占位凭证的 OAuth 参考实现不应被宿主启动强制安装），core 为 `builtin` 保证依赖可满足，已记入 design D5
...
- [x] 8.1 更新 `apps/lina-core/pkg/plugin/README.md` 与 `README.zh-CN.md`：Auth 域行改述为 provider SPI 引擎（`ProvideExternalIdentityProvider` 声明、linapro-extlogin-core 归属、fail-closed 语义），移除 `sys_user_external_identity` 宿主独占表述
- [x] 8.2 更新 `apps/lina-plugins/README.md` 与 `README.zh-CN.md` 插件清单：加入 `linapro-extlogin-core`，顺带补齐缺失的 `linapro-oidc-google`/`linapro-oidc-discord` 行
- [x] 8.3 编写 `linapro-extlogin-core` 双语 `README.md`/`README.zh-CN.md`（能力边界、宿主边界、数据权限边界、开户契约、目录结构、审查清单）；同步修正 google/discord 双语 README 的过时表述（"宿主负责开户"→ 委托 core、依赖声明、审查清单）
- [x] 8.4 PR 描述内容（供创建 PR 时使用）：external-identity 存储与开户策略已从 host core 抽离为 `linapro-extlogin-core` 插件——宿主仅保留薄 `extlogin` 接口 + `extidspi` SPI + manager 惰性注入（启用门禁 fail-closed）；`sys_user_external_identity` 表及 DAO 工件已删除；两个 follow-up（无邮箱确定性 anchor 开户、绑定/解绑/列举自隔离 API）已随插件闭环；google/discord 仅新增对 core 的依赖声明，登录调用路径零改动
...
- [x] **FB-11**: `linapro-extlogin-core` 插件私有表名不符合项目规范（应为 `plugin_<plugin-id>_*`）
  ...
  - **修复**：表重命名为 `plugin_linapro_extlogin_core_user_external_identity`；索引缩短为 `uk_plugin_linapro_extlogin_core_identity_provider_subject` / `idx_plugin_linapro_extlogin_core_identity_user`（规避 PG 63 字符标识符上限）；同步安装/卸载 SQL、`hack/config.yaml`（`removePrefix`）、DAO/DO/Entity、identity 单测建表、插件 README、OpenSpec D2/spec/proposal
...
- [x] **FB-10**: Google/Discord 插件介绍描述去技术化，改为产品化官方插件文案
  - **根因**：`plugin.yaml` / `plugin.json` 仍写 “external-identity seam / 外部身份接缝 / 参考源码插件”，与 `linapro-extlogin-core` 及内容/监控类官方插件「Official source plugin for … / 提供…的官方源码插件」风格不一致
...
- [x] **FB-6**: 英文环境下 Google/Discord OIDC 设置菜单仍显示中文
  - **根因**：宿主 `localizeMenuEntity` 按 `menu.<menuKey>.title` 投影菜单名；`linapro-extlogin-core` 的「授权登录」目录已补 `menu.json`，但 `linapro-oidc-google`/`linapro-oidc-discord` 仅有 `plugin.json`/`error.json`，缺少 `manifest/i18n/{en-US,zh-CN}/menu.json`。翻译未命中时回退库内 `sys_menu.name`；历史安装或中文源文案写入后，英文环境仍显示中文
...
- [x] **FB-5**: 第三方授权登录插件设置菜单统一挂到「授权登录」目录（由 extlogin-core 安装创建，非宿主预留）
  - **根因**：Google/Discord 原挂 `extension`；一度误用宿主种子目录 `auth-login`，违背“领域目录随领域插件安装”的边界
  - **修复**：`linapro-extlogin-core` 声明 `plugin:linapro-extlogin-core:auth-login` 目录菜单；Google/Discord `parent_key` 指向该目录；**不新增/不保留宿主 SQL**（已删除误加的 `013-auth-login-catalog.sql`）
  - **影响分析**：i18n=有（extlogin-core `menu.plugin:linapro-extlogin-core:auth-login` en/zh）；缓存=无；数据权限=无；开发工具=无；测试=E2E TC-18
  - **验证**：目录仅在 extlogin-core 安装后出现；OIDC 设置挂其下；E2E 已拆到插件目录（extlogin-core TC001 / google+discord TC001），宿主 TC018 与 TC006 对 OIDC 的硬耦合已移除
...
- [x] **FB-1**: `linapro-extlogin-core` 插件介绍信息（name/description）缺少 i18n，插件管理页中文环境回退英文
  ...
  - **影响分析**：i18n=有影响（插件介绍键 `plugin.linapro-extlogin-core.{name,description}`，en-US+zh-CN）；缓存一致性=无影响；数据权限=无影响；开发工具跨平台=无影响；测试策略=JSON 合法性 + `make i18n.check` 治理验证（无运行时逻辑变更，无需 E2E/单测）；DI=无新增运行期依赖
  - **验证**：`python3` JSON 解析通过；键存在性断言通过；`make i18n.check` exit 0
- [x] **FB-2**: `linapro-extlogin-core` 插件介绍文案过技术化，需更地道、自然、通用的产品描述
  ...
  - **影响分析**：i18n=有影响（`plugin.linapro-extlogin-core.description` en-US/zh-CN + `plugin.yaml` 源文本）；缓存=无；数据权限=无；开发工具=无；测试策略=JSON/文案静态校验（纯文案，无行为变更）
