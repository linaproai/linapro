...
- **BREAKING**（仅对 `feat/plugin-auth` 分支内未发布代码）：`google`/`discord` 插件对 `extlogin` seam 的依赖不变，但外部身份存储改由 `linapro-extid-core` 提供，二者新增对 `linapro-extid-core` 的运行时依赖（provider 未启用时 external login fail-closed）。
- 收敛两个已知 follow-up 到插件内：放宽无邮箱开户（派生用户名/替代锚点）与新增 `BindVerifiedIdentity` 绑定/解绑能力，均落在 `linapro-extid-core` 内部，宿主不再参与具体策略。
