...
- **新增** `apps/lina-core/pkg/plugin/capability/authcap/externallogin/externalidentityspi/externalidentityspi.go`
...
- **改** `apps/lina-core/internal/service/auth/auth.go`
  - 接口加 `BindExternalIdentityProvider(provider externalidentityspi.Provider)`
  - `serviceImpl` 加字段 `identityProvider externalidentityspi.Provider`（nil = fail-closed）
  - `ExternalProvisionInput` 加 `UsernameAnchor` 字段
  - import 加 `externalidentityspi`
...
- **⚠️ 预存在阻塞（非本任务，需组 2 附近顺手修）**：`go vet ./...` 有 2 个 `authcap.New` arity 错误——`wasm_host_service_test.go:664`、`integration_test.go:125`。组 1 auth SPI 给 `authcap.New` 加了第三参 `externallogin.Service`，但这两个测试文件的 `authcap.New(...)` 调用点未同步（`git stash` 对照证明与 1b.3 无关）。

### 4 — 新建 linapro-oidc-core 插件（在 apps/lina-plugins 子仓库）
...
- **验收硬证据**：`apps/lina-plugins/linapro-oidc-core/plugin.yaml` 必须存在于磁盘。

### 5 — 插件实现 provider SPI + Bind/Unbind/List 交付面 + i18n

- 在 `backend/internal/service/` 实现 `externalidentityspi.Provider`：
...
- `linapro-oidc-google`、`linapro-oidc-discord` 的 `plugin.yaml` 声明依赖 `linapro-oidc-core`。
- 确认两者登录路径不变（仍走 `externallogin` seam），core 未启用时 external login fail-closed。
...
- `apps/lina-plugins/README.md` 插件清单加 `linapro-oidc-core`。
