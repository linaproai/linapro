...
- `hack/scripts/install/checks/prereq.sh` 在 `install.sh` 安装链路末尾被调用,检查 `go ≥ 1.22` / `node ≥ 20` / `pnpm ≥ 8` / `git` / `make` / `mysql client` 的存在性与版本,占用 5666 / 8080 端口给 warning,缺失工具时只打印平台特定的安装提示文本(如 `brew install go`),**不动手安装**
...
- 安装入口收敛为 `install.sh` 单文件下载入口:只负责把仓库源码下载到本地,不再执行 `prereq.sh` 探测、平台脚本分发、运行环境安装或项目初始化动作
- `lina-doctor` 仅服务"开发态环境治理"场景,不替代 `install.sh` 的"下载仓库源码到本地"职责
...
- 跨平台覆盖三套环境:`macOS` / `Linux` / `Windows-via-Git-Bash-or-WSL`,与 `install.sh` 一致
...
- **G6** 不引入新的二进制 CLI、不引入新的项目级配置文件,但**承担**`install.sh` 卸下来的环境探测职责(原 `prereq.sh` 范围 + 新增 `openspec` / `gf` / `Playwright` / `goframe-v2`)
...
- **NG2** 不在本变更中处理 "用户尚未克隆仓库" 的环境(那是 `install.sh` 的职责);`lina-doctor` 默认假设运行在已克隆的仓库根目录下
...
- **NG8** 不在 `install.sh` 链路中硬调用 `lina-doctor`;两条路径解耦,避免循环依赖

## Decisions

### D1 技能运行范围: 仅"克隆后"环境治理,不替代 install.sh

**决策**: `lina-doctor` 假设运行在已克隆的 `LinaPro` 仓库根目录(检测 `apps/lina-core` 与 `apps/lina-vben` 是否存在),不承担"克隆仓库"职责。仅"克隆后"场景纳入;"克隆前/源码下载"由 `install.sh` 的单文件下载入口负责,但 `install.sh` 不做运行环境探测和安装。

**为什么**:

- `install.sh` 单文件设计已承诺"零二级 curl",将 `lina-doctor` 拉进克隆前流程会破坏该约束
- `Playwright` 浏览器安装必须在 `hack/tests/` 内执行 `pnpm exec`,本质上要求仓库已存在
- 用户原始诉求是"安装了框架后,可能缺少运行环境",已显式指向克隆后场景
- 解耦后两个工具职责清晰:`install.sh` = 下载仓库源码;`lina-doctor` = 工具链治理

**替代方案与拒绝理由**:

- *让 `lina-doctor` 也能克隆仓库*: 与 `install.sh` 职责重叠,引入双入口分歧;拒绝
- *让 `install.sh` 在环境缺失时自动拉起 `lina-doctor`*: 破坏单文件下载入口、引入 skill 路径反向依赖;拒绝
...
- *只支持 PowerShell 终端运行 lina-doctor*: 与 `install.sh` 的"Git Bash / WSL"约定冲突;拒绝
...
### D13 install 脚本职责收敛: install.sh 仅负责下载仓库源码

**决策**: `hack/scripts/install/install.sh` 收敛为"解析目标版本 + git clone 下载仓库源码 + 打印 next-steps"的单文件入口;平台判断仅用于给出不支持环境的快速失败提示,**完全不再触发**任何运行环境探测、依赖安装、项目初始化、端口探测。三个平台分发脚本(`install-macos.sh` / `install-linux.sh` / `install-windows.sh`)、`prereq.sh`、`lib/_common.sh` 整体从仓库删除。再次发布的 `https://linapro.ai/install.sh` 内容同步更新。

默认安装使用完整 Git 克隆,通过 `git ls-remote --tags --refs` 解析远程最高稳定 `vX.Y.Z` 标签并检出该标签,同时保留 `origin` 远程地址和标签获取配置。这样后续升级可以直接在已安装目录中执行 `git fetch --tags --force origin` 并切换到新发布标签。`LINAPRO_SHALLOW=1` 仅作为显式体积优化选项保留,脚本必须提示浅克隆首次标签升级时可能需要 `git fetch --unshallow --tags --force origin`。
...
- 单一脚本(`install.sh`)更便于 CDN 部署与 review;减少对 `lib/_common.sh` 的依赖也让 `install.sh` 自包含语义更彻底
- 保留正常 Git 仓库语义比只下载压缩包或一次性浅检出更适合后续按标签升级
...
**缓解**:
- `install.sh` 克隆完成后输出的 next-steps 提示必须显著、可复制,清晰说明"clone 已成功 → 调用 lina-doctor → 跑 make init"三步
...
- 重写 `hack/scripts/install/install.sh` 为"探测 OS + 解析版本 + git clone + 打印 next-steps"四步
