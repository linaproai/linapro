...
- 新增“智能中心”官方源码插件 `linapro-ai-core`，提供供应商、模型管理、能力档位和调用日志页面。
- 供应商页面直接维护供应商及其支持模型，支持添加、编辑、删除、同步或手工维护模型，并保护被能力档位引用的供应商和模型。
- 能力档位页面固定提供 `basic`、`standard`、`advanced` 三档文本生成能力配置，每档绑定一个主供应商模型，并以 `capabilityType + capabilityMethod + tierCode` 作为唯一身份，为后续图片、音频、向量等能力方法保留扩展边界。
...
- `linapro-ai-core-plugin`：定义官方智能中心插件的供应商、模型管理、档位管理、调用日志、插件自有存储、前端页面和权限治理。
...
- 新增插件自有表，建议使用 `plugin_linapro_ai_provider`、`plugin_linapro_ai_model`、`plugin_linapro_ai_tier`、`plugin_linapro_ai_tier_binding`、`plugin_linapro_ai_invocation` 等插件命名空间；模型、档位和调用日志必须保存 `capability_type` 与 `capability_method`。
