...
- [x] 3.5 链接表 schema：subject_kind、app_context、phone/display/avatar 等扩展字段已并入 `001-linapro-extlogin-core-identities.sql`（删除冗余 `002` ALTER）
...
- [x] 4.1 确认 dependencies 指向 linapro-extlogin-core；README 安装顺序
...
- [x] **FB-2**: 安装 `linapro-extlogin-core` 时 `001-linapro-extlogin-core-identities.sql` 失败：开发库残留旧表缺少扩展列，`CREATE TABLE IF NOT EXISTS` 空操作后 `COMMENT ON COLUMN` 报错
