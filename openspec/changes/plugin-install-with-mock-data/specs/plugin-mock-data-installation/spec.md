...
宿主在手动安装路径（`POST /plugins/{id}/install`）的请求 DTO 中 SHALL 提供 `installMockData bool` 字段，默认值为 `false`。当请求体显式设置 `installMockData=true` 时，宿主 SHALL 在 install SQL 全部执行成功后，扫描该插件 `manifest/sql/mock-data/` 目录中的 SQL 文件并按文件名顺序加载执行；当字段未设置或为 `false` 时，宿主 MUST 完全跳过 mock-data 目录的扫描与执行。前端 plugin 安装弹窗 SHALL 提供"是否安装示例数据"复选框，仅在该插件存在 `manifest/sql/mock-data/` 目录（即 `hasMockData=true`）时显示，复选框默认未勾选；说明入口 SHALL 使用与文字垂直对齐的问号图标承载 tooltip，不再展示"了解详情"文字；该区域 SHALL 紧跟插件基础信息表格之后展示，避免在动态插件授权、路由等长内容后被放到弹窗底部。勾选行为本身即视为用户对加载示例数据的知情同意，前端与后端 MUST NOT 引入额外的环境栅栏、独立权限或二次确认步骤。

#### Scenario: 用户勾选复选框成功安装示例数据

- **WHEN** 用户在插件安装弹窗中勾选"是否安装示例数据"复选框并提交
...
- **WHEN** 用户在插件安装弹窗中未勾选"是否安装示例数据"复选框并提交
- **AND** 该插件 `manifest/sql/mock-data/` 目录下存在合法的 SQL 文件
...
- **THEN** "是否安装示例数据"复选框 MUST 不渲染到 DOM 中
...
- **AND** 用户在前端弹窗中勾选"是否安装示例数据"安装该动态插件
...
### Requirement: 插件列表必须用独立列展示示例数据可用性

插件管理列表 SHALL 在状态列与安装时间列之间展示"示例数据"列。该列 MUST 使用插件列表接口返回的 `hasMockData` 元数据作为唯一来源：带示例数据的插件显示绿色标签"是"，不带示例数据的插件显示灰色标签"否"。插件名称列 MUST NOT 再追加"示例数据"标识，避免名称、auto-enable 标识与示例数据状态混杂。

插件列表标题 SHALL 在"插件列表"文字后展示问号帮助图标。用户悬停该图标时，前端 SHALL 展示常见说明，至少包含源码插件与动态插件的差异，以及"示例数据"列中"是"/"否"的含义。

#### Scenario: 插件列表清晰展示示例数据状态

- **WHEN** 用户打开插件管理列表
- **AND** 某插件的 `hasMockData=true`
- **THEN** 该插件在"示例数据"列显示绿色标签"是"
- **AND** 该插件名称后不显示示例数据标识

#### Scenario: 不带示例数据的插件显示否

- **WHEN** 用户打开插件管理列表
- **AND** 某插件的 `hasMockData=false`
- **THEN** 该插件在"示例数据"列显示灰色标签"否"

#### Scenario: 插件列表标题提供常见说明

- **WHEN** 用户打开插件管理列表
- **AND** 用户悬停"插件列表"标题后的问号图标
- **THEN** 前端展示源码插件与动态插件差异说明
- **AND** 前端展示"示例数据"列中"是"/"否"的含义说明

### Requirement: 卸载确认必须突出清理插件自有存储数据的风险并执行硬删除

插件卸载弹窗中"同时清理插件自有存储数据"选项 SHALL 放入具有明显警示效果的区域中展示，使用警告或危险色背景提醒用户勾选后会删除插件自有表数据与授权存储文件。用户确认卸载且该选项生效时，宿主 SHALL 执行插件卸载 SQL、授权存储目录清理、菜单清理与资源引用清理；其中资源引用和插件自有数据清理 MUST 采用硬删除或等价的物理删除语义，不得仅通过 `deleted_at` 或状态字段软删除后保留僵尸数据。

#### Scenario: 用户确认卸载并清理插件自有存储数据

- **WHEN** 用户在卸载弹窗中保持"同时清理插件自有存储数据"选项勾选并确认
- **THEN** 前端在确认前以警示区域突出该清理选项
- **AND** 后端执行插件卸载 SQL 或授权存储清理来物理删除插件自有数据
- **AND** 治理资源引用不保留软删除僵尸行
