...
type UpdateDemoRecordReq struct {
	g.Meta                  `path:"/demo-records/{id}" method:"put" tags:"动态插件示例" summary:"更新动态插件示例记录" dc:"更新一条 plugin-demo-dynamic 示例记录，并可替换或移除当前附件，用于演示动态插件页面对插件自有数据表与授权存储文件的修改操作" access:"login" permission:"plugin-demo-dynamic:record:update" operLog:"update"`
