...
type CreateDemoRecordReq struct {
	gmeta.Meta              `path:"/demo-records" method:"post" tags:"动态插件示例" summary:"创建动态插件示例记录" dc:"创建一条 plugin-demo-dynamic 示例记录，并可同时上传一个插件自有附件文件，用于演示动态插件页面对安装 SQL 创建的数据表与授权存储文件的写入操作" access:"login" permission:"plugin-demo-dynamic:record:create" operLog:"create"`
