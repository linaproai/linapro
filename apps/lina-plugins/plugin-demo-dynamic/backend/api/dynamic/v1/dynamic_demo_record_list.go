...
type DemoRecordListReq struct {
	g.Meta   `path:"/demo-records" method:"get" tags:"动态插件示例" summary:"分页查询动态插件示例记录" dc:"分页查询 plugin-demo-dynamic 示例页面中的业务记录，支持按标题关键字模糊筛选，用于演示动态插件安装 SQL 创建的数据表在页面中的增删查改操作" access:"login" permission:"plugin-demo-dynamic:record:view" operLog:"other"`
