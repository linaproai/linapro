...
(0, '登录展示-忘记密码入口', 'sys.auth.forgetPasswordEnabled', 'true', 1, '控制登录页是否显示忘记密码入口，可选值：true、false。关闭后访问忘记密码路由会回退到登录页。', NOW(), NOW()),
(0, '登录展示-创建账号入口', 'sys.auth.registerEnabled', 'true', 1, '控制登录页是否显示创建账号入口，可选值：true、false。关闭后访问注册路由会回退到登录页。', NOW(), NOW()),
(0, '登录展示-隐私政策正文', 'sys.auth.privacyPolicy', E'隐私政策\n\n本服务仅为账号认证与工作区访问收集必要信息，包括用户名与邮箱。数据仅用于提供宿主工作台及相关安全能力。如需了解数据保留或导出，请联系系统管理员。', 1, '控制注册页“隐私政策”弹窗正文，支持多行文本。', NOW(), NOW()),
(0, '登录展示-服务条款正文', 'sys.auth.termsOfService', E'服务条款\n\n创建账号即表示您同意按照所属组织的规定使用本工作区。因滥用或安全风险，账号可能被停用。运营方可更新本条款；在通知后继续使用视为接受更新。', 1, '控制注册页“服务条款”弹窗正文，支持多行文本。', NOW(), NOW()),
