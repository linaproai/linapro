...
var productionPanicPolicy = panicAuditPolicy{
	...
	Allowances: []panicAllowance{
		...
		},
		{
			...
			Function: "normalizePluginAutoEnableIDs",
			Count:    1,
			Category: panicCategoryStaticConfig,
			Reason:   "startup auto-enable plugin declarations must be non-empty",
		},
		{
			Path:     "apps/lina-core/internal/service/config/config_plugin.go",
			Function: "validatePluginAutoEnableRawValue",
			Count:    3,
			Category: panicCategoryStaticConfig,
			Reason:   "startup plugin auto-enable configuration must have the published array shape",
		},
		{
