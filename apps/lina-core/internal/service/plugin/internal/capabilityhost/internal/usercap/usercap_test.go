...
func TestSearchUsersCountsWithoutProjectionFields(t *testing.T) {
	...
	sqls, err := gdb.CatchSQL(ctx, func(sqlCtx context.Context) error {
		_, searchErr := New(nil, nil).SearchUsers(sqlCtx, capmodel.CapabilityContext{}, capabilityusercap.SearchInput{
...
func TestBatchGetUsersAppliesDataScope(t *testing.T) {
	...
	result, err := New(nil, scopeSvc).BatchGetUsers(ctx, capmodel.CapabilityContext{}, []capabilityusercap.UserID{"7", "8"})
	if err != nil {
