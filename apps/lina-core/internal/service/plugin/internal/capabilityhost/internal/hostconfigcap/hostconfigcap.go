// Package hostconfigcap adapts host runtime configuration rows to governed
...
var (
	_ capabilityhostconfigcap.AdminService = (*adapter)(nil)
)
...
func (a *adapter) BatchGetRuntimeConfig(ctx context.Context, _ capmodel.CapabilityContext, keys []capabilityhostconfigcap.RuntimeConfigKey) (*capmodel.BatchResult[*capabilityhostconfigcap.RuntimeConfigProjection, capabilityhostconfigcap.RuntimeConfigKey], error) {
...
func (a *adapter) SetRuntimeConfigJSON(ctx context.Context, _ capmodel.CapabilityContext, key capabilityhostconfigcap.RuntimeConfigKey, valueJSON []byte) error {
	normalizedKey := strings.TrimSpace(string(key))
	if normalizedKey == "" {
