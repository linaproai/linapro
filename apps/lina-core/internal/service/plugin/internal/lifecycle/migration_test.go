...
func TestExecuteManifestSQLFilesRollsBackSQLAndLedgerOnSQLFailure(t *testing.T) {
	...
	err = services.Lifecycle.ExecuteManifestSQLFiles(ctx, manifest, catalog.MigrationDirectionInstall)
	if err == nil {
	...
	assertTestTableMissing(t, ctx, tableName)
	assertMigrationRowCount(t, ctx, pluginID, catalog.MigrationDirectionInstall, 0)
}
...
func TestExecuteManifestSQLFilesRollsBackSQLOnLedgerFailure(t *testing.T) {
	...
	oversizedPhase := catalog.MigrationDirection(strings.Repeat("ledger-failure-", 8))
	err = services.Lifecycle.ExecuteManifestSQLFiles(ctx, manifest, oversizedPhase)
	if err == nil {
...
func assertMigrationRowCount(t *testing.T, ctx context.Context, pluginID string, phase catalog.MigrationDirection, expected int) {
	t.Helper()
