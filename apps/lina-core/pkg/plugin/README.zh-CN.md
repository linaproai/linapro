...
| `Auth` | 通过`Token()`、`Authz()`和`ExternalLogin()`子能力处理租户 token 选择或切换、代用户 token、权限投影、单权限判断、批量权限判断和外部身份登录。`ExternalLogin()`仅面向源码插件：宿主自行盖章调用插件身份，强制`ProvideExternalIdentity`的 provider 归属与插件启用检查，token 与会话铸造保持宿主独占；动态插件收到 fail-closed 桩实现。 | 运行期检查当前用户、租户、权限 key 和方法级授权。外部登录通过由某个源码插件（`linapro-oidc-core`）经`ProvideExternalIdentityProvider`声明的`externalidentityspi.Provider`引擎解析`(provider, subject)`；链接存储与开户策略归该插件所有。未安装启用引擎插件时外部登录 fail-closed：不解析链接、不开户。 |
