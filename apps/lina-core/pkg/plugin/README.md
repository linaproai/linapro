...
| `Auth`/`Authz` | Handles tenant token selection or switching, impersonation tokens, permission projections, single permission checks, and batch permission checks. | Runtime checks use current user, tenant, permission keys, and method-level authorization. |
