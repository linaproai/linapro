...
| `Auth` | Handles tenant token selection or switching, impersonation tokens, permission projections, single permission checks, batch permission checks, and external-identity login through `Token()`, `Authz()`, and `ExternalLogin()` sub-capabilities. `ExternalLogin()` is source-plugin-only: the host stamps the calling plugin identity, enforces `ProvideExternalIdentity` provider ownership plus plugin enablement, and keeps token and session minting host-owned; dynamic plugins receive a fail-closed stub. | Runtime checks use current user, tenant, permission keys, and method-level authorization. External login resolves `(provider, subject)` through the `externalidentityspi.Provider` engine declared by one source plugin (`linapro-oidc-core`) via `ProvideExternalIdentityProvider`; linkage storage and provisioning policy live in that plugin. When no engine plugin is installed and enabled, external login is fail-closed: no linkage resolves and no account is provisioned. |
