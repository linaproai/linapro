...
1. 对照 `AGENTS.md` 的 Bugfix 反馈测试要求，确认每个 `bugfix` 反馈修复都有能够复现原始问题并验证修复有效性的自动化测试。
2. 后端纯逻辑、服务层、工具函数、缓存、权限、数据权限、插件桥接等内部行为修复，必须检查是否新增或更新了对应单元测试，并确认相关测试命令已运行通过。
3. 涉及用户可观察页面、路由、表单、表格、权限交互、接口联动或端到端工作流的修复，必须检查是否新增或更新了对应 E2E 测试，并确认相关 E2E 与识别出的回归测试已运行通过。
4. 如果 `bugfix` 反馈修复缺少单元测试或 E2E 测试，或没有运行对应测试的证据，必须报告为严重问题；此类任务不得在审查结论中视为完成。
...
### Bugfix 反馈测试覆盖审查
✓ 每个 bugfix 反馈都有单元测试或 E2E 测试覆盖 / ⚠ 发现 N 个缺失测试覆盖问题
