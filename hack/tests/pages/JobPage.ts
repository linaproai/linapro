...
export class JobPage {
  ...
  async fillCommonFields(params: {
    ...
    if (params.cronExpr) {
      const cron = this.dialog.getByLabel('Cron 表达式');
      await cron.clear();
      await cron.fill(params.cronExpr);
    }
    if (params.timezone) {
      const timezone = this.dialog.getByLabel('任务时区');
      await timezone.clear();
      await timezone.fill(params.timezone);
    }
  }
  ...
  async isPausedByPluginVisible() {
    return this.page.getByText('插件不可用', { exact: true }).first().isVisible();
  }
