...
const apiBaseURL =
  process.env.E2E_API_BASE_URL ?? new URL('/api/v1/', config.baseURL).toString();
...
export async function listGroups(api: APIRequestContext, keyword = '') {
  return expectSuccess<{ list: GroupListItem[]; total: number }>(
    await api.get(`job-group?pageNum=1&pageSize=100&code=${encodeURIComponent(keyword)}`),
  );
}

export async function getDefaultGroup(api: APIRequestContext): Promise<GroupListItem> {
...
export async function listHandlers(api: APIRequestContext) {
  return expectSuccess<{ list: HandlerListItem[] }>(await api.get('job/handler'));
}

export async function getMenuIdsByPerms(api: APIRequestContext, perms: string[]) {
  const result = await expectSuccess<{ list: MenuNode[] }>(await api.get('menu'));
...
export function buildHandlerJobPayload(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    groupId: 1,
...
export function buildShellJobPayload(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    groupId: 1,
...
export async function createJob(api: APIRequestContext, payload: Record<string, unknown>) {
  return expectSuccess<{ id: number }>(await api.post('job', { data: payload }));
}

export async function updateJob(api: APIRequestContext, id: number, payload: Record<string, unknown>) {
...
export async function listJobs(api: APIRequestContext, keyword = '') {
  return expectSuccess<{ list: JobDetail[]; total: number }>(
    await api.get(`job?pageNum=1&pageSize=100&keyword=${encodeURIComponent(keyword)}`),
  );
}

export async function updateJobStatus(api: APIRequestContext, id: number, status: 'enabled' | 'disabled') {
...
export async function listLogs(api: APIRequestContext, jobId?: number) {
  const query = jobId ? `?pageNum=1&pageSize=100&jobId=${jobId}` : '?pageNum=1&pageSize=100';
  return expectSuccess<{ list: LogDetail[]; total: number }>(await api.get(`job/log${query}`));
}
...
export async function previewCron(api: APIRequestContext, expr: string, timezone: string) {
...
export async function getConfigByKey(api: APIRequestContext, key: string) {
  return expectSuccess<ConfigItem>(await api.get(`config/key/${encodeURIComponent(key)}`));
}

export async function updateConfigValue(api: APIRequestContext, id: number, value: string) {
  await expectSuccess(await api.put(`config/${id}`, { data: { value } }));
}

export async function setCronShellEnabled(api: APIRequestContext, enabled: boolean) {
...
export function buildPayloadFromJob(detail: JobDetail): Record<string, unknown> {
  return {
