...
test.describe("TC-117 默认品牌 Logo 资源", () => {
  test("TC-117a: 公共前端配置返回本地图标 Logo", async ({ request }) => {
    ...
    expect(payload.data.app.logo).toBe("/linapro-mark.png");
    expect(payload.data.app.logoDark).toBe("/linapro-mark.png");
  });
  ...
  test("TC-117b: 登录页使用图标 Logo 且渲染应用名文本", async ({
    ...
    expect(logo.src).toBe("/linapro-mark.png");
    expect(logo.currentSrc).toContain("/linapro-mark.png");
    expect(logo.naturalWidth).toBe(64);
    expect(logo.naturalHeight).toBe(63);
