...
func commandRegistry() map[string]commandSpec {
	specs := []commandSpec{
		...
		{Name: "skills", Description: "Aggregate menu for skills.link / skills.unlink (interactive on TTY).", Usage: "linactl skills", Run: runSkills},
		{Name: "skills.link", Description: "Manage repository-local symlinks from supported agents' project skill paths to .agents/skills.", Usage: "linactl skills.link [agent=<name|all|csv>] [force=1]", Run: runSkillsLink},
		{Name: "skills.unlink", Description: "Remove repository-local symlinks managed by skills.link.", Usage: "linactl skills.unlink agent=<name|all|csv>", Run: runSkillsUnlink},
